...
bool Tester2::test1(){
    ...
    auto a = librarian.search_books_checked_out(0,"","Design Patterns","","",2003,0,0,0);
   // librarian.return_book([0].first.check_out_id);
...
bool Tester2::test2(){
    ...
    if(p1.get_checked_out_books()[0].first.fine != 0)
        return false;
    if(p1.get_checked_out_books()[1].first.day_end != 2 || p1.get_checked_out_books()[1].first.month_end != 4) //wrong ans
        return false;
    if(p1.get_checked_out_books()[1].first.fine != 0)
        return false;
    /*if(s.get_checked_out_books()[0].first.day_end != 26 || p1.get_checked_out_books()[0].first.month_end != 3) //wrong ans
        return false;*/
    if(s.get_checked_out_books()[0].first.fine == 0)
        return false;
    /*if(s.get_checked_out_books()[1].first.day_end != 19 || p1.get_checked_out_books()[1].first.month_end != 3) //wrong ans
        return false;*/
    if(s.get_checked_out_books()[1].first.fine == 0)
        return false;
    if(v.get_checked_out_books()[0].first.day_end != 12 || p1.get_checked_out_books()[0].first.month_end != 3)
        return false;
    if(v.get_checked_out_books()[0].first.fine == 0)
        return false;
    if(v.get_checked_out_books()[1].first.day_end != 12 || p1.get_checked_out_books()[1].first.month_end != 3)
        return false;
