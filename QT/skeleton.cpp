...
public:
    ...
    QVector<Book> search_books(QString authors, QString title, QString keywords, QString publisher, int year, bool bestseller, bool available, bool or_and){
        ...
        while (query.next()) {
            ...
            ans.push_back(Book(authors, title, keywords, publisher, id, year, copies, price, room ,level, bestseller));
        }
        return ans;
    }
    ...
    Book get_book(int id){
        ...
        int copies = query.value(9).toInt();
        bool bestseller = query.value(10).toInt();
        return Book(authors, title, keywords, publisher, id, year, copies, price, room ,level, bestseller);
    }
...
public:
    PatronUser login_patron(QString login, QString password){
        ...
        if (query.next()){
            ...
            QString user_login = query.value(5).toString();
    ...
    void login_librarian(QString login, QString password){
        QSqlQuery query;
